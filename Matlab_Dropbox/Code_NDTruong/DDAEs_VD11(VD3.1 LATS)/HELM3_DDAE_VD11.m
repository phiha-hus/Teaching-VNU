...
for i =1:m
 ...
 %========= GT ban dau cua Dao Ham EX====================
   D(no+1)= lamda*exp(lamda*t(no+1));  D(no+2)= lamda*exp(lamda*t(no+2)); 
 %  tinh nghiem xap xy tai tung ðiem mot.=========================================
    for j=no+4:M+no+3
     % --- Dung noi suy---------------------------------
        VP1= U(j-1) - w*t(j-1)*V(j-1) + (beta2*D(j-2) + beta3*D(j-3))*h(i) + lamda*beta1*(U(j-1) - w*t(j-1)*V(j-1))*h(i) + a*beta1*(noisuy1(4,tau,t(j-1),h(i),t,V) - exp(lamda*t(j-1) - lamda))*h(i);
        ...
        U(j)= (1 + w*t(j))*V(j) + VP2;
        D(j-1)=(U(j) - U(j-1) - w*t(j)*V(j) + w*t(j-1)*V(j-1))/(beta1*h(i)) - (beta2*D(j-2) + beta3*D(j-3))/beta1;
        SSU(j)=abs(UCX(j)-U(j));
        SSV(j)=abs(VCX(j)-V(j));
    end;
