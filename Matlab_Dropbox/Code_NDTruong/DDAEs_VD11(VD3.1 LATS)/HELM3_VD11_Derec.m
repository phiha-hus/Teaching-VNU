function HELM3_VD11_Derec
%%-----------PP HEAdamBashforth 2 buoc, Bac 2 ap dung cho BT da bien doi c?a VD10.=====================
clear all
format short e;
h0=input('nhap buoc thoi gian dau h0 = ');
%w=input('nhap w = ');
w=10; lamda = -1.5; a=0.5;  b= 1; c= 0.8;
% ========== He so HELM3, 3 buoc, order 2 ==================
...
for i =1:m
 ...
 %========= GT ban dau cua Dao Ham EX====================
   DV(no+1)= lamda*exp(lamda*t(no+1));  DV(no+2)= lamda*exp(lamda*t(no+2)); 
   DU(no+1)= (lamda + w + lamda*w*t(no+1))*exp(lamda*t(no+1));  DU(no+2)= (lamda + w + lamda*w*t(no+2))*exp(lamda*t(no+2)); 
 %  tinh nghiem xap xy tai tung ðiem mot.=========================================
    for j=no+4:M+no+3
     % --- Dung noi suy---------------------------------
        VP1= U(j-1) - w*t(j-1)*V(j-1) + (beta2*DU(j-2) + beta3*DU(j-3))*h(i) - w*t(j-1)*(beta2*DV(j-2) + beta3*DV(j-3))*h(i)+ lamda*beta1*U(j-1)*h(i) - beta1*h(i)*w*(lamda*t(j-1) - 1)*V(j-1) + a*beta1*(noisuy1(4,tau,t(j-1),h(i),t,V) - exp(lamda*t(j-1) - lamda))*h(i);
        VP2= b*noisuy1(4,tau,t(j),h(i),t,U) + (c - b*w*t(j) + b*w)*noisuy1(4,tau,t(j),h(i),t,V) - (b + c)*exp(lamda*t(j) - lamda);
        V(j)= (VP1 - VP2)/(1 + w*h(i)); %U(j)= w*t(j)*V(j) + VP1;%
        U(j)= (1 + w*t(j))*V(j) + VP2;
        DU(j-1)=(U(j) - U(j-1))/(beta1*h(i)) - (beta2*DU(j-2) + beta3*DU(j-3))/beta1;
        DV(j-1)=(V(j) - V(j-1))/(beta1*h(i)) - (beta2*DV(j-2) + beta3*DV(j-3))/beta1;
        SSU(j)=abs(UCX(j)-U(j));
        SSV(j)=abs(VCX(j)-V(j));
    end;
...
plot(t,UCX);hold on
plot(t,U,'+ M');
title(' Do hoa nghiem xap xy X_1 (-) va nghiem CX X_1 (+)');
subplot(2,2,4);
plot(t,VCX);hold on
plot(t,V,'+ M');
title(' Do hoa nghiem xap xy X_2 (-) va nghiem CX X_2 (+)');
% Lap cac ham NOI SUY
