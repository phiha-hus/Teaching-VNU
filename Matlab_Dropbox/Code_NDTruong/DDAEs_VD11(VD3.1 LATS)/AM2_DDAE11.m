...
for i =1:m
 ...
 %========= GT ban dau cua Dao Ham EX====================
    D(no+2)= lamda*exp(lamda*t(no+2)); D(no+3)= lamda*exp(lamda*t(no+3));
 %  tinh nghiem xap xy tai tung ðiem mot.=========================================
    for j=no+4:M+no+3
     % --- Dung noi suy---------------------------------
        VP1= U(j-1) - w*t(j-1)*V(j-1)  + (beta1*D(j-1) + beta2*D(j-2))*h(i) + a*beta0*(noisuy1(5,tau,t(j),h(i),t,V) - exp(lamda*t(j) - lamda*tau))*h(i);
        VP2= b*noisuy1(5,tau,t(j),h(i),t,U) + (c - b*w*t(j) + b*w*tau)*noisuy1(5,tau,t(j),h(i),t,V) - (b + c)*exp(lamda*t(j) - lamda*tau);
        V(j)= VP1/(1 - lamda*beta0*h(i)) - VP2;
        U(j)= (1 + w*t(j))*V(j) + VP2;
        D(j)=(U(j) - U(j-1) - w*t(j)*V(j) + w*t(j-1)*V(j-1))/(beta0*h(i)) - (beta1*D(j-1) + beta2*D(j-2))/beta0;
        SSU(j)=abs(UCX(j)-U(j));
        SSV(j)=abs(VCX(j)-V(j));
    end;
