...
for k =1:m
    ...
    for i=3:N
        %----- HELM3 method
        VP =  U(i)*U(i) + U(i)*V(i)*(t(i)*t(i)+ 2*sin(t(i))) + U(i)*(beta2*DH(i-1) + beta3*DH(i-2))*h(k) + h(k)*beta1*V(i)*U(i)*(exp(-t(i)) + 2*t(i) + 2*cos(t(i))) + h(k)*beta1*exp(-t(i))*t(i)*t(i)*cos(t(i))  + h(k)*beta1*U(i)*sin(2*t(i)) - h(k)*beta1*(V(i)+ 1)*exp(-2*t(i));
        %----- Adams Basford 2
        %VP =  U(i)*U(i) + U(i)*V(i)*(t(i)*t(i)+ 2*sin(t(i))) + U(i)*(beta2*DH(i-1))*h(k) + h(k)*beta1*V(i)*U(i)*(exp(-t(i)) + 2*t(i) + 2*cos(t(i))) + h(k)*beta1*exp(-t(i))*t(i)*t(i)*cos(t(i))  + h(k)*beta1*U(i)*sin(2*t(i)) - h(k)*beta1*(V(i)+ 1)*exp(-2*t(i));
        %---------------------------
        V(i+1) = (U(i)*(sin(t(i+1)) - 1)*exp(-t(i+1)) + VP)/(U(i)*(exp(-t(i+1)) + t(i+1)*t(i+1) + 2*sin(t(i+1))));
        U(i+1) = V(i+1)*exp(-t(i+1)) - (exp(-t(i+1)))*(sin(t(i+1)) - 1); 
        %----- HELM3 method
        DH(i)= (U(i+1) + (t(i+1)*t(i+1) + 2*sin(t(i+1)))*V(i+1) - U(i) - (t(i)*t(i) + 2*sin(t(i)))*V(i))/(beta1*h(k)) - (beta2*DH(i-1))/beta1 - (beta3*DH(i-2))/beta1;      
        %----- Adams Basford 2
        %DH(i)= (U(i+1) + (t(i+1)*t(i+1) + 2*sin(t(i+1)))*V(i+1) - U(i) - (t(i)*t(i) + 2*sin(t(i)))*V(i))/(beta1*h(k)) - (beta2*DH(i-1))/beta1;      
        %------------------------------
