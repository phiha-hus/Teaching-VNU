...
for k =1:m
    ...
    for i=2:N
        ...
        dem = 0; maxci=1;
         ...
         A(1,1) = 2*TGD(1) + TGD(2)*(t(i+1)*t(i+1) + 2*sin(t(i+1))- h(k)*beta0*(2*t(i+1) + 2*cos(t(i+1)) + exp(-t(i+1)))) + alpha1*U(i) + alpha1*V(i)*(t(i)*t(i) + 2*sin(t(i))) + alpha2*U(i-1) + alpha2*V(i-1)*(t(i-1)*t(i-1) + 2*sin(t(i-1))) - h(k)*beta0*sin(2*t(i+1)); 
...
D=x1*x1 + x1*y1*(t*t + 2*sin(t) - h1*bt0*(2*t + 2*cos(t) + exp(-t))) + x1*(alp1*x2 + alp2*x3 + alp1*y2*((t-h1)*(t-h1) + 2*sin(t-h1)) + alp2*y3*((t-2*h1)*(t-2*h1) + 2*sin(t-2*h1))) - h1*bt0*(t*t*cos(t) - exp(-t))*exp(-t) + h1*bt0*(y1*exp(-2*t) - x1*sin(2*t));
