...
clear all
format short e;
beta0 = 1/2; beta1 = 1/2;% Trapezoidal Method; Order p = 2
...
for k =1:m
    ...
    for i=1:N
        ...
        dem = 0; maxci=1;
         ...
         A(1,1) = 2*TGD(1) + TGD(2)*(t(i+1)*t(i+1) + 2*sin(t(i+1)) - h(k)*beta0*(2*t(i+1) + 2*cos(t(i+1))+ exp(-t(i+1)))) - U(i) - (t(i)*t(i) + 2*sin(t(i)))*V(i) - (beta0*sin(2*t(i+1)) + beta1*DH(i))*h(k); 
         A(1,2) = TGD(1)*(t(i+1)*t(i+1) + 2*sin(t(i+1)) - h(k)*beta0*(2*t(i+1) + 2*cos(t(i+1)) + exp(-t(i+1)))) + h(k)*beta0*exp(-2*t(i+1));
         A(2,1) = exp(t(i+1)); A(2,2)= -1; 
         %JND = inv(A);
         F(1)= FA1(TGD(1),U(i),TGD(2),V(i),DH(i),beta0,beta1,t(i+1),h(k));
        ...
        DH(i+1)= (U(i+1) + (t(i+1)*t(i+1) + 2*sin(t(i+1)))*V(i+1) - U(i) - (t(i)*t(i) + 2*sin(t(i)))*V(i))/(beta0*h(k)) - (beta1*DH(i))/beta0;      
...
function D=FA1(x1,x2,y1,y2,dh1,bt0,bt1,t,h1)
D=x1*x1 + x1*y1*(t*t + 2*sin(t) - h1*bt0*(2*t + 2*cos(t) + exp(-t))) - x1*(x2 + y2*((t-h1)*(t - h1) + 2*sin(t-h1)) + h1*(bt0*sin(2*t) + bt1*dh1)) + h1*bt0*y1*exp(-2*t) - h1*bt0*exp(-t)*(t*t*cos(t) - exp(-t));
