...
{
  int m = s->m;               // Order of interpolation, m+1 points
  ...
  /**
   * TODO: Evaluate the interaction of the cluster 's' via tensor-interpolation
   */
...
{
  const uint size = s->size; // Number of bodies within the cluster 's'
  const int *idx = s->idx;   // Permutation array for this cluster
  const real *y1 = st->y[0]; // x-coordinates of all bodies
  const real *y2 = st->y[1]; // y-coordinates of all bodies
  const real *y3 = st->y[2]; // z-coordinates of all bodies

  real result;

  /**
   * TODO: evaluate all interaction from the bodies with the cluster 's'
   */
                  ...
                  cluster *s, const state *st, real eta)
{
  ...
  /**
   * TODO: Evaluate the potential for a cluster 's'
   */
