...
void solve_gridfunc2d(pgridfunc2d x)
{
    ...
    clear_gridfunc2d(r);
    axpy(n2, 1.0, x->x, 1, r->x, 1);
    addeval_5point_stencil_gridfunc2d(r, -1.0, x);
    ...
    {
        clear_gridfunc2d(a);
        addeval_5point_stencil_gridfunc2d(a, 1.0, p);

        gamma = dot(n2, p->x, 1, a->x, 1);
        lambda = dot(n2, p->x, 1, r->x, 1) / gamma;

        axpy(n2, lambda, p->x, 1, x->x, 1);
        axpy(n2, -lambda, a->x, 1, r->x, 1);

        mu = dot(n2, r->x, 1, a->x, 1) / gamma;
        scal(n2, -mu, p->x, 1);
        axpy(n2, 1.0, r->x, 1, p->x, 1);
