...
void explicit_euler(pgridfunc2d x, pgridfunc2d v, real delta, real t_start, real t_end)
{
    ...
    while (t < t_end)
    {
        printf("\rt = %.3f s", t);
        clear_gridfunc2d(Ax);
        addeval_5point_stencil_gridfunc2d(Ax, 1.0, x);

        add_gridfunc2d(x, delta, v);
        add_gridfunc2d(v, -delta*wave_c, Ax);
...
void leapfrog(pgridfunc2d x, pgridfunc2d v, real delta, real t_start, real t_end)
{
    ...
    i = 0;
    printf("\rt = %.3f s", t);
    addeval_5point_stencil_gridfunc2d(v, -delta, x);
    ...
    while (t < t_end)
    {
        ...
        add_gridfunc2d(x, 2.0 * delta, v);
        addeval_5point_stencil_gridfunc2d(v, -2.0 * delta * wave_c, x);
