function [x,n] = newton(f,df,x0,nmax,eps,delta,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function newton is an implementation of Newton method for solving f(x)=0
% Form: [x,n] = newton(f,df,x0,nmax,eps,delta,varargin)
% f: function 
% df: the derivatice function
% x0 : starting point 
% nmax: maximal number of steps to be taken
% eps: maximal allowable error between two consecutive iterations
...
% between two consecutive iterates is smaller than eps
...
if nargin ==3
  nmax = 1000; eps = 1e-6; delta = 1e-12;
...
for i=1:nmax
  %if abs(y)<= eps       % Do not use this, since the number of significant ...
                         % ... figures in x is not guarantee
  if abs(diff) <= eps   
    disp('Newton method converges since |x_{n+1} - x_n| < eps.')
    break
  else
